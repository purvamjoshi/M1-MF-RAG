...
class MutualFundRetriever {
  ...
  async initialize() {
    ...
    // Try to load FAISS index (optional - will fallback if unavailable)
    try {
      ...
      const faissIndexPath = path.join(indexDir, 'faiss-index.bin');
      const faissMappingPath = path.join(indexDir, 'faiss-mapping.json');
      
      if (fs.existsSync(faissIndexPath) && fs.existsSync(faissMappingPath)) {
        console.log('Loading FAISS vector index...');
        
        this.faissIndex = IndexFlatL2.read(faissIndexPath);
        this.faissMapping = JSON.parse(fs.readFileSync(faissMappingPath, 'utf8'));
        
        console.log(`✓ Loaded FAISS index with ${this.faissMapping.length} vectors`);
        this.initialized = true;
        console.log('✓ Retriever initialized with vector search');
      } else {
        console.warn('⚠️  FAISS index not found - using direct lookup');
  ...
  async retrieve(query, options = {}) {
    ...
    // If FAISS is not available, use direct lookup
    if (!this.faissIndex || !this.embeddingModel) {
    ...
    try {
      ...
      // Search FAISS index
      const k = Math.min(limit * 3, this.faissMapping.length); // Get more candidates for filtering
      const searchResult = this.faissIndex.search(queryEmbedding, k);
      
      // searchResult contains: { labels: [indices], distances: [distances] }
      const indices = searchResult.labels;
      const distances = searchResult.distances;
      
      // Map indices to chunks and apply filters
      let results = [];
      for (let i = 0; i < indices.length; i++) {
        const idx = indices[i];
        if (idx >= 0 && idx < this.faissMapping.length) {
          const mapping = this.faissMapping[idx];
          const chunk = this.chunkLookup[mapping.chunk_id];
          
          if (chunk) {
            // Apply filters
            if (schemeId && mapping.scheme_id !== schemeId) continue;
            if (sectionType && mapping.section_type !== sectionType) continue;
            
            results.push({
              ...chunk,
              vectorScore: 1 / (1 + distances[i]), // Convert distance to similarity score
              vectorDistance: distances[i]
            });
          }
        }
      }
      ...
      if (results.length === 0 && (schemeId || sectionType)) {
        for (let i = 0; i < indices.length; i++) {
          const idx = indices[i];
          if (idx >= 0 && idx < this.faissMapping.length) {
            const mapping = this.faissMapping[idx];
