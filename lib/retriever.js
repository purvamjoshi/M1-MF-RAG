  ...
  async initialize() {
    ...
    try {
      ...
      this.metadataIndex = JSON.parse(
        fs.readFileSync(path.join(indexDir, 'metadata-index-latest.json'), 'utf8')
      );
      
      // Load chunk lookup
      this.chunkLookup = JSON.parse(
        fs.readFileSync(path.join(indexDir, 'chunk-lookup-latest.json'), 'utf8')
      );
      ...
      throw new Error('Retriever initialization failed. Run build-index.js first.');
    }
  }
