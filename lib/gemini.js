...
ANSWER (conversational, friendly, factual, no advice):`;
  ...
  extractDirectAnswer(query, chunk) {
    ...
    // But ALLOW factual questions about holdings, portfolios, etc.
    const advisoryPatterns = [
      /best.*fund.*(?:invest|buy)|which.*fund.*(?:should|better|invest)/i,
      /should.*(?:buy|invest).*fund|recommend.*fund.*(?:invest|buy)/i,
      /good.*investment|worth.*investing|safe.*(?:to invest|investment)/i,
      /(?:give|need|want).*advice|advise.*me|your.*opinion/i,
    ];
