...
ANSWER (conversational, friendly, factual, no advice):`;
  ...
  extractDirectAnswer(query, chunk) {
    ...
    const advisoryPatterns = [
      /best.*fund|which.*fund.*best|top.*fund|better.*fund/i,
      /should.*buy|should.*invest|recommend.*fund|suggest.*fund/i,
      /good.*investment|worth.*investing|safe.*fund/i,
      /advice|advise|opinion|view.*on/i,
    ];
